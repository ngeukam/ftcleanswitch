...
import AddApartmentDialog from "../ApartmentInfo/AddApartmentDialog";
...
function PropertyApartments() {
  ...
  const generateColumns = (data: Apartment[]) => {
    ...
    const baseColumns: GridColDef[] = [
      ...
      },
      {
        ...
        renderCell: (params: GridRenderCellParams) =>
          `${params.row.currency}${" "}${
            params.row.price?.toLocaleString() || "0.00"
          }`,
      },
      {
        field: "status",
  ...
  return (
    <Box sx={{ display: "flex" }}>
      <Appbar
        appBarTitle={`${property[0]?.name} - ${property[0]?.address}: Apartments`}
      />
      <Box
        component="main"
        sx={{
      ...
      >
        ...
        <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
          ...
          <DeleteConfirmationDialog
            ...
            itemName={
              selectedApartment ? `Apartment #${selectedApartment.number}` : ""
            }
